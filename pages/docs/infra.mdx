...
The models, built primarily in PyTorch, are converted to the ONNX format, which allows us to run them anywhere with the ONNX runtime. Specifically, we're using the ONNX runtime for Javascript to run the models in the browser. The models are loaded into the client's browser, where it is run using the WASM (Web Assembly) version of the ONNX runtime.
